...
{
    public abstract class SimpleTransformBlock<TInput, TOutput> : IPropagatorBlock<TInput, TOutput>
    {
        private readonly IPropagatorBlock<TInput, TOutput> _block;

        protected SimpleTransformBlock(ExecutionDataflowBlockOptions options, bool throwOnException = true)
        {
            _block = new TransformManyBlock<TInput, TOutput>(input =>
            {
                try
                {
                    return new[] { Handle(input) };
                }
                catch
                {
        ...
        internal SimpleTransformBlock(IPropagatorBlock<TInput, TOutput> block)
        {
